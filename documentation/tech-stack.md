...
- **Accessibility**: Applied across the app: stable IDs and `htmlFor` on form labels (login, pet form, settings, users), ARIA where needed (`role="main"`, `role="navigation"`, `role="tablist"`/`tab`/`tabpanel`, `role="alert"` for errors, `role="status"` for loading/success), `aria-label`/`aria-labelledby`/`aria-describedby`, `aria-invalid` and `aria-busy` on controls, and semantic structure (`<main>`, `<nav>`, `<footer role="contentinfo">`, `<aside role="complementary">`). Decorative icons use `aria-hidden`. ComboBox exposes `role="combobox"` with `aria-expanded` and `aria-haspopup="listbox"`. This keeps the accessibility tree clear for screen readers and automation.
